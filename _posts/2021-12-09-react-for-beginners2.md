...
`setState(counter +1)`처럼 값을 넣어준다면, state를 변수의 값으로 업데이트 해준다.
...
그 이유는 만약 한 번에 setState가 여러 번 불렸다고 생각해보자.
setState가 동기함수라면 값을 업데이트 하고 랜더링을 한다.
그런데 setState를 여러 번 호출했으므로 랜더링도 여러 번 일어난다.
...
코드가 굉장히 길 경우, setState로 동일한 일을 처리할 수도 있다.
